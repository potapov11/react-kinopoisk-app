...
function Card(props) {
	...
	lsFavorArray.forEach((item) => {
		if (item.filmId === film.filmId) {
			film.favorite = true;
		}
	});

	function handleFavoriteBlock(film) {
		if (!isCheckedFavorit) {
			setToFavoriteArray(film);
		} else {
			removeFromFavoriteArray(film);
		}
		setIsChekedFavorit((prevSt) => !prevSt);
	}

	return (
		<div className="movie" onClick={(e) => showModal(e, film.filmId)}>
			<div className="like" onClick={() => handleFavoriteBlock(film)}>
				<svg className="like__svg" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
					...
					<g id="SVGRepo_iconCarrier">
						<path
							...
							className={isCheckedFavorit || film.favorite ? 'checked' : null}
							fillRule="evenodd"
							clipRule="evenodd"
